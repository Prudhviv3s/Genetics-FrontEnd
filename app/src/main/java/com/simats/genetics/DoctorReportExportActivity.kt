...
class DoctorReportExportActivity : AppCompatActivity() {
    ...
    private fun setupExportMethods() {
        ...
        findViewById<View>(R.id.card_print).apply {
            ...
            setOnClickListener {
                Toast.makeText(this@DoctorReportExportActivity, "Printing not implemented yet", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        ApiClient.getApi(this).getExportInfo(analysisId).enqueue(object : Callback<ExportInfoResponse> {
            override fun onResponse(
                ...
                if (!response.isSuccessful) {
                    Toast.makeText(this@DoctorReportExportActivity, "Failed (${response.code()})", Toast.LENGTH_LONG).show()
                    return
                }
                ...
                if (body?.status != true || exportData == null) {
                    Toast.makeText(this@DoctorReportExportActivity, body?.error ?: "No info", Toast.LENGTH_LONG).show()
                    return
                }
            ...
            override fun onFailure(call: Call<ExportInfoResponse>, t: Throwable) {
                showLoading(false)
                Toast.makeText(this@DoctorReportExportActivity, t.message ?: "Network error", Toast.LENGTH_LONG).show()
            }
        })
    }
    ...
    private fun openShareScreen(mode: String) {
        val i = Intent(this, DoctorReportShareActivity::class.java)
